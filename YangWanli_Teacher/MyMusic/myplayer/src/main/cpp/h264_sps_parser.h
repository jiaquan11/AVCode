...
#ifndef MYPLAYER_H264_SPS_PARSER_H_
#define MYPLAYER_H264_SPS_PARSER_H_
...
static void parseh264_sps(uint8_t *sps, uint32_t sps_size, int *level,
                          int *profile, bool *interlaced, int32_t *max_ref_frames) {
    nal_bitstream bs;
...
static bool ff_avpacket_is_idr(const AVPacket *pkt) {
    ...
    if (pkt->data && pkt->size >= 5) {
        ...
        while (offset >= 0 && offset + 5 <= pkt->size) {
            ...
            offset += (bytesToInt((uint8_t *) nal_start) + 4);
        }
    }
    return false;
}
...
#endif //MYPLAYER_H264_SPS_PARSER_H_
