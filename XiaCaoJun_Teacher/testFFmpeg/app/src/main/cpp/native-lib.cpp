...
Java_com_example_testffmpeg_MainActivity_stringFromJNI(JNIEnv *env, jobject /* this */) {
...
Java_com_example_testffmpeg_MainActivity_Open(JNIEnv *env, jobject thiz, jstring urlStr, jobject handle) {
...
Java_com_example_testffmpeg_XPlay_Open(JNIEnv *env, jobject thiz, jstring urlStr, jobject surface) {
    ...
    for (int i = 0; i < ic->nb_streams; ++i) {
        ...
        if (as->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
            ...
            LOGI("fps= %d, width=%d height=%d codecId=%d", fps, as->codecpar->width, as->codecpar->height, as->codecpar->codec_id);
        } else if (as->codecpar->codec_type == AVMEDIA_TYPE_AUDIO) {
            LOGI("音频数据");
            audioStream = i;
            LOGI("sample_rate: %d channels: %d sample_format: %d", as->codecpar->sample_rate, as->codecpar->channels, as->codecpar->format);
        }
    }
...
////////////////////////////////////////////////////////////////////////////////
    ...
    for (;;) {
        ...
        //发送到线程中解码 内部会对传入的packet进行缓存，所以avcodec_send_packet执行完糊可以直接清理传入的packet
            ...
            if (cc == vc) {
                ...
                if (!vctx) {
                    ...
                    if (h > 0) {
                        ...
                        ANativeWindow_unlockAndPost(nwin);//显示图像
                    }
                }
            } else {//音频
                uint8_t *out[2] = {0};
